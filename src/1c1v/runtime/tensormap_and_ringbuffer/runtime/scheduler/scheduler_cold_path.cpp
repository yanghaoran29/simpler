...
void SchedulerContext::log_stall_diagnostics(
            ...
            for (int32_t si = 0; si < ring_task_count; si++) {
                ...
                if (is_running) {
                    ...
                    LOG_INFO_V9(
                        ...
                        " state=RUNNING fanin_refcount=%d/%d kernels=[aic:%d aiv0:%d] "
                        "running_on=[owner_thread=%d cores=[%s]]",
                        thread_idx, idle_iterations, r, task_id, rc, fi, kid_aic, kid_aiv0, owner, running_on
                    );
                    continue;
                }
                if (rc >= fi) {
                    ...
                    LOG_INFO_V9(
                        "[STALL thread=%d idle_iterations=%d] TASK ring=%d task_id=%" PRId64
                        " state=READY   fanin_refcount=%d/%d kernels=[aic:%d aiv0:%d]",
                        thread_idx, idle_iterations, r, task_id, rc, fi, kid_aic, kid_aiv0
                    );
                    continue;
                }
                cnt_waiting++;
                ...
                LOG_INFO_V9(
                    "[STALL thread=%d idle_iterations=%d] TASK ring=%d task_id=%" PRId64
                    " state=WAIT    fanin_refcount=%d/%d kernels=[aic:%d aiv0:%d] missing_deps=%d",
                    thread_idx, idle_iterations, r, task_id, rc, fi, kid_aic, kid_aiv0, fi - rc
                );
            }
        }
    ...
    for (int32_t cli = 0; cli < tracker.get_cluster_count() && cli < STALL_DUMP_CORE_MAX; cli++) {
        int32_t offset = cli * 2;
        int32_t aic_id = tracker.get_aic_core_id(offset);
        int32_t aiv0_id = tracker.get_aiv_core_id(offset);
        bool aic_idle = tracker.is_aic_core_idle(offset);
        bool aiv0_idle = tracker.is_aiv_core_idle(offset);
        int32_t cluster_id = cli * ast + thread_idx;
        char aic_buf[128], aiv0_buf[128];
        format_core_status(
        ...
        LOG_INFO_V9(
            "[STALL thread=%d idle_iterations=%d] CLUSTER cluster_id=%d aic=%s aiv0=%s", thread_idx, idle_iterations,
            cluster_id, aic_buf, aiv0_buf
        );
    }
}
...
int32_t SchedulerContext::handshake_all_cores(Runtime *runtime) {
    ...
    // 1C1V (sim): the platform launches exactly 1 AIC + 1 AIV per cluster, so
    // every handshake worker is a real schedulable core (no parked AIV1).
    // Worker layout: AIC in [0, block_dim), AIV in [block_dim, 2*block_dim).
...
bool SchedulerContext::assign_cores_to_threads() {
    ...
    // 1C1V: each cluster = 1 AIC + 1 AIV (AIV0); the pair is always kept together.
    ...
    int32_t thread_cores_num = max_clusters_per_thread * 2;
    ...
    for (int32_t ci = 0; ci < cluster_count; ci++) {
        ...
        // 1C1V: discovery skipped parked AIV1, so aiv_worker_ids_ is dense (AIV0 only).
        int32_t aiv0_wid = aiv_worker_ids_[ci];

        core_trackers_[t].set_cluster(cluster_idx_per_thread[t]++, aic_wid, aiv0_wid);

        LOG_INFO_V0("Thread %d: cluster %d (AIC=%d, AIV=%d)", t, ci, aic_wid, aiv0_wid);
    }
...
void SchedulerContext::reassign_cores_for_all_threads() {
    ...
    for (int32_t ci = 0; ci < cluster_count; ci++) {
        ...
        core_trackers_[t].set_cluster(cl_idx, aic_wid, aiv0_wid);

        // init() marks all idle; toggle cores that were running and restore pending_occupied
        if (running_cores[aic_wid]) {
            core_trackers_[t].change_core_state(cl_idx * 2);
            core_trackers_[t].set_pending_occupied(cl_idx * 2);
        }
        if (running_cores[aiv0_wid]) {
            core_trackers_[t].change_core_state(cl_idx * 2 + 1);
            core_trackers_[t].set_pending_occupied(cl_idx * 2 + 1);
        }
    }
...
int32_t SchedulerContext::init(
    ...
    for (int32_t t = 0; t < sched_thread_num_; t++) {
        ...
        for (int32_t c = 0; c < tracker.get_cluster_count(); c++) {
            int32_t cluster_offset = c * 2;  // Each cluster = 1 AIC + 1 AIV
            auto aiv0_id = tracker.get_core_id_by_offset(tracker.get_aiv_core_offset(cluster_offset));
