...
//
// 1C1V: each cluster is 1 AIC + 1 AIV. core_states_ encodes per-cluster core
// idle/running in 2 bits per cluster:
//   bit i*2   = AIC of cluster i   (1 = idle, 0 = running)
//   bit i*2+1 = AIV of cluster i
// Max 31 clusters per tracker (63 bits in uint64_t).
// =============================================================================

class alignas(64) CoreTracker {
public:
    static inline int32_t MAX_CORE_PER_THREAD = 63;
    static constexpr int32_t MAX_CLUSTERS = 63 / 2;
...
public:
    void init(int32_t cluster_count) {
        ...
        for (int32_t i = 0; i < cluster_count; i++) {
            aic_mask_ |= BitStates(1ULL << (i * 2));
            aiv_mask_ |= BitStates(2ULL << (i * 2));
        }
        core_states_ = aic_mask_ | aiv_mask_;
    }

    void set_cluster(int32_t cluster_idx, int32_t aic_wid, int32_t aiv_wid) {
        core_id_map_[cluster_idx * 2] = aic_wid;
        core_id_map_[cluster_idx * 2 + 1] = aiv_wid;
    }
    ...
    BitStates get_valid_cluster_offset_states(PTO2ResourceShape shape) const {
        ...
        case PTO2ResourceShape::AIV:
            return (core_states_ >> 1) & aic_mask_;
        case PTO2ResourceShape::MIX:
            return (core_states_ >> 1) & core_states_ & aic_mask_;
    ...
    int32_t get_aiv_core_id(int32_t cluster_offset) const { return core_id_map_[cluster_offset + 1]; }

    int32_t get_aic_core_offset(int32_t cluster_offset) const { return cluster_offset; }
    int32_t get_aiv_core_offset(int32_t cluster_offset) const { return cluster_offset + 1; }

    bool is_aic_core_idle(int32_t cluster_offset) const {
        return ((core_states_ >> cluster_offset) & BitStates(1ULL)).has_value();
    }
    bool is_aiv_core_idle(int32_t cluster_offset) const {
    ...
    // For AIV: 1 bit per AIV core (1 bit per cluster at aiv_mask_ positions).
    ...
    // MIX: 1 bit per cluster where BOTH cores have free pending slots AND at least one is running.
    ...
    BitStates get_pending_core_offset_states(PTO2ResourceShape shape) const {
        if (shape == PTO2ResourceShape::MIX) {
            ...
            BitStates mix_available = (available & aic_mask_) & ((available >> 1) & aic_mask_);
            // Exclude fully-idle clusters (handled by IDLE phase) to prevent double-dispatch.
            BitStates running = ~core_states_;
            BitStates cluster_has_running = (running & aic_mask_) | ((running >> 1) & aic_mask_);
    ...
    int32_t core_num() const { return cluster_count_ * 2; }
...
private:
    ...
    int32_t core_id_map_[63];  // bit_position -> worker_id, max 31 clusters * 2
};
