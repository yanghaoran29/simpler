...
void SchedulerContext::dispatch_subtask_to_core(
    ...
    LOG_DEBUG(
        "Thread %d: Dispatched %s %s task %" PRId64 " kernel_id=[%d,%d] block_idx=%d/total_blocks=%d to"
        ...
        slot_state.task->kernel_id[1], block_idx, slot_state.logical_block_num, core_offset, core_id, reg_task_id
    );
...
void SchedulerContext::dispatch_mix_block_to_cluster(
    ...
    if (cmask & PTO2_SUBTASK_MASK_AIV0) {
        bool aiv0_to_pending = to_pending && !tracker.is_aiv_core_idle(cluster_offset);
        dispatch_subtask_to_core(
            thread_idx, tracker.get_aiv_core_offset(cluster_offset), slot_state, PTO2SubtaskSlot::AIV0,
...
void SchedulerContext::dispatch_shape(
    ...
    while (cores.has_value() && !entered_drain) {
        ...
        PTO2TaskSlotState *batch[CoreTracker::MAX_CLUSTERS * 2];
