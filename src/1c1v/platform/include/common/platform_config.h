 ...
 */
constexpr int PLATFORM_MAX_BLOCKDIM = 120;

/**
 * Core composition per block dimension
 * 1C1V (sim): 1 block = 1 AIC cube + 1 AIV. No paired AIV1 — the simulator
 * launches exactly one AIV per cluster.
 */
constexpr int PLATFORM_CORES_PER_BLOCKDIM = 2;
constexpr int PLATFORM_AIC_CORES_PER_BLOCKDIM = 1;
constexpr int PLATFORM_AIV_CORES_PER_BLOCKDIM = 1;

/**
 * Maximum AICPU scheduling threads
 * Determines parallelism level of the AICPU task scheduler.
 */
constexpr int PLATFORM_MAX_AICPU_THREADS = 12;
 ...
 * from scheduling while still participating in affinity (e.g. 14 launch, 12 active).
 */
 ...
 * - MAX_AIC_PER_THREAD = MAX_BLOCKDIM * AIC_CORES_PER_BLOCKDIM = 120 * 1 = 120
 * - MAX_AIV_PER_THREAD = MAX_BLOCKDIM * AIV_CORES_PER_BLOCKDIM = 120 * 1 = 120
 */
constexpr int PLATFORM_MAX_AIC_PER_THREAD = PLATFORM_MAX_BLOCKDIM * PLATFORM_AIC_CORES_PER_BLOCKDIM;  // 120

constexpr int PLATFORM_MAX_AIV_PER_THREAD = PLATFORM_MAX_BLOCKDIM * PLATFORM_AIV_CORES_PER_BLOCKDIM;  // 120

constexpr int PLATFORM_MAX_CORES_PER_THREAD = PLATFORM_MAX_AIC_PER_THREAD + PLATFORM_MAX_AIV_PER_THREAD;  // 240
 ...
 * Calculated as: MAX_BLOCKDIM * CORES_PER_BLOCKDIM = 120 * 2 = 240
 */
constexpr int PLATFORM_MAX_CORES = PLATFORM_MAX_BLOCKDIM * PLATFORM_CORES_PER_BLOCKDIM;  // 240
 ...
 * 1C1V (sim): 1 cluster = 1 AIC + 1 AIV
 */
constexpr uint32_t PLATFORM_SUB_CORES_PER_AICORE = PLATFORM_CORES_PER_BLOCKDIM;

/**
 * Maximum physical AIC (cluster) count. 1C1V sim models 120 clusters, so this
 * must be >= PLATFORM_MAX_BLOCKDIM; physical core count = this * CORES_PER_BLOCKDIM.
 */
namespace DAV_2201 {
constexpr uint32_t PLATFORM_MAX_PHYSICAL_CORES = 120;
}
