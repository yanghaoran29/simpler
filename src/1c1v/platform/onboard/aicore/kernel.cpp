...
#ifdef __DAV_VEC__
    // 1C1V: the hardware couples 2 AIV per cube, but only AIV0 of each cluster is
    // scheduled. The parked AIV1 (subblockid==1) returns immediately so it never
    // enters the executor handshake spin (AICPU skips it during discovery).
    if (get_subblockid() == 1) {
        return;
    }
