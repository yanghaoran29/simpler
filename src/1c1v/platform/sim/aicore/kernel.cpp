...
extern "C" void aicore_execute_wrapper(
    ...
    // 1C1V core layout in sim DeviceRunner (no paired AIV1):
    //   physical_core_id [0..block_dim-1]          = AIC of cluster i
    //   physical_core_id [block_dim..2*block_dim-1] = AIV of cluster i
    // Each cluster is 1 AIC + 1 AIV, so subblock_id is always 0.
    ...
    if (core_type == CoreType::AIC) {
        ...
        cluster_id = physical_core_id - block_dim;
    }
