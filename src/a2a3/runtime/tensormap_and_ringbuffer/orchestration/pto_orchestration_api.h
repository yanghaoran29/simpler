...
/** This-run AIC (= cluster) count. Do not hardcode 24/36; MIX cohorts use this. */
static inline int32_t rt_available_aic_counts() {
    PTO2Runtime *rt = current_runtime();
    return rt->ops->available_cluster_count(rt);
}

/** This-run AIV core count (= 2 × clusters on standard silicon). */
static inline int32_t rt_available_aiv_counts() {
    PTO2Runtime *rt = current_runtime();
