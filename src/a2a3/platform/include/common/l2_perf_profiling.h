...
struct L2PerfRecord {
    ...
    // L2PerfRecord at its original cache-line-aligned size (avoids shrinking
    // records-per-buffer when records are sized in bytes).
    //
    // unlocked_count: number of fanout consumers whose fanin_refcount reached
    //   fanin_count as a direct result of THIS task's completion (i.e.
    //   consumers this TaskDone pushed to the ready queue). Sourced from
    //   CompletionStats::tasks_enqueued in on_mixed_task_complete. For
    //   non-final subtasks of a mixed task, this is 0 (only the last subtask
    //   carries the unlocks).
    //
    ...
    //   wired (so they never appended this task to their fanout_head). The
    //   "true" producer→consumer edge count for an analysis is:
    //     recorded_fanout + sum_consumer(early_finished_count)
