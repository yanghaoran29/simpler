...
bool SchedulerContext::has_idle_in_other_threads(int32_t self_thread_idx, ResourceShape shape) const {
    ...
    ChipReadyQueue *continuation = continuation_ready_queues(self_thread_idx);
    ...
    ChipReadyQueue *local = local_ready_queues(self_thread_idx);
...
void SchedulerContext::dispatch_ready_tasks(
    ...
    ChipReadyQueue *global_queues = sched_->domain_ready_queues[static_cast<int32_t>(TaskReadyDomain::GLOBAL)];
    ...
    if (!die_routing_active_cache_[thread_idx]) {
        run_staging_order(
            ...
            [&] {
                return global_queues[static_cast<int32_t>(ResourceShape::MIX)].size() > 0;
            }
        );
        return;
    }
    ...
    if (sched_->continuation_routing_active[thread_idx]) {
        ChipReadyQueue *continuation_queues = continuation_ready_queues(thread_idx);
        run_staging_order(
            thread_idx, pmu_active,
            [&](ResourceShape shape, CoreTracker::DispatchPhase phase) {
                dispatch_shape(
            ...
            [&] {
                return continuation_queues[static_cast<int32_t>(ResourceShape::MIX)].size() > 0;
            }
        );
        if (entered_drain) return;
    ...
    ChipReadyQueue *local_queues = local_ready_queues(thread_idx);
    run_staging_order(
        thread_idx, pmu_active,
        [&](ResourceShape shape, CoreTracker::DispatchPhase phase) {
            ...
            ChipReadyQueue *first = global_first || !has_local ? global_queues : local_queues;
            ChipReadyQueue *second = first == global_queues ? local_queues : global_queues;
    ...
    while (true) {
        ...
        if (rt_ != nullptr && rt_->aicore_mailbox != nullptr &&
            ...
            AsyncPollResult poll_result = sched_->async_wait_list.poll_and_complete<false>(
                ...
                thread_idx, thread_ready_domain(thread_idx)
            );
