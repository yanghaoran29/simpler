...
bool die_aware_core_assignment(int32_t mode, int32_t active_threads) {
    return sched_assignment_base_mode(mode) == kSchedAicoreAssignmentDieAware && active_threads == 4;
}

bool hybrid_die_aware_core_assignment(int32_t mode, int32_t active_threads) {
    return sched_assignment_base_mode(mode) == kSchedAicoreAssignmentHybridDieAware && active_threads == 4;
}

int32_t pthread_for_cluster(int32_t ci, int32_t cluster_count, int32_t active_threads, int32_t assignment_mode) {
    return logical_sched_for_cluster(ci, cluster_count, active_threads, assignment_mode);
}

bool cluster_owned_by_thread(
    int32_t ci, int32_t tidx, int32_t cluster_count, int32_t active_threads, int32_t assignment_mode
) {
    return cluster_owned_by_pthread(ci, tidx, cluster_count, active_threads, assignment_mode);
}
...
void SchedulerContext::handshake_owned_clusters(Runtime *runtime, int32_t tidx, int32_t active_threads) {
    ...
    for (int32_t ci = 0; ci < aic_n; ++ci) {
        if (!cluster_owned_by_thread(ci, tidx, aic_n, active_threads, sched_aicore_assignment_mode_)) continue;
        owned[own_n++] = ci;                  // AIC
...
void SchedulerContext::assign_own_clusters(int32_t tidx) {
    ...
    int32_t own_n = 0;
    for (int32_t ci = 0; ci < aic_n; ++ci) {
        if (!cluster_owned_by_thread(ci, tidx, aic_n, active, sched_aicore_assignment_mode_)) continue;
        own_n++;
    }
    ...
    for (int32_t ci = 0; ci < aic_n; ++ci) {
        if (!cluster_owned_by_thread(ci, tidx, aic_n, active, sched_aicore_assignment_mode_)) continue;
        tracker.set_cluster(local++, ci, aic_n + 2 * ci, aic_n + 2 * ci + 1, aicore_cluster_die(ci, aic_n));
    }
...
bool SchedulerContext::assign_cores_to_threads() {
    ...
    const int32_t base_assignment_mode = sched_assignment_base_mode(sched_aicore_assignment_mode_);
    if (base_assignment_mode == kSchedAicoreAssignmentRoundRobin) {
        LOG_INFO(
    ...
    } else if (hybrid_die_aware_core_assignment(sched_aicore_assignment_mode_, active_sched_threads_)) {
        LOG_INFO(
            "Assigning cores (hybrid die-aware): %d clusters across %d sched threads — "
            "sched 0 die0 only, sched 3 die1 only, sched 1-2 split both dies (%d AIC, %d AIV)",
            cluster_count, active_sched_threads_, aic_count_, aiv_count_
        );
    ...
    for (int32_t ci = 0; ci < cluster_count; ci++) {
        clusters_per_thread[pthread_for_cluster(
            ci, cluster_count, active_sched_threads_, sched_aicore_assignment_mode_
        )]++;
    }
    ...
    for (int32_t ci = 0; ci < cluster_count; ci++) {
        int32_t t = pthread_for_cluster(ci, cluster_count, active_sched_threads_, sched_aicore_assignment_mode_);

        int32_t aic_wid = aic_worker_ids_[ci];
        int32_t aiv0_wid = aiv_worker_ids_[2 * ci];
        int32_t aiv1_wid = aiv_worker_ids_[2 * ci + 1];

        core_trackers_[t].set_cluster(
            cluster_idx_per_thread[t]++, aic_wid, aiv0_wid, aiv1_wid, aicore_cluster_die(ci, cluster_count)
        );
...
int32_t SchedulerContext::pre_handshake_init(
    ...
    dependency_die_affinity_enabled_ = sched_assignment_uses_dependency_die_affinity(sched_aicore_assignment_mode_);
    explicit_data_affinity_enabled_ = sched_assignment_uses_explicit_data_affinity(sched_aicore_assignment_mode_);
    regs_ = regs_base;
...
void SchedulerContext::deinit() {
    ...
    sched_aicore_assignment_mode_ = kSchedAicoreAssignmentSequential;
    dependency_die_affinity_enabled_ = false;
    explicit_data_affinity_enabled_ = false;
    for (int32_t t = 0; t < MAX_AICPU_THREADS; t++) {
...
void SchedulerContext::bind_runtime(PTO2Runtime *rt) {
    rt_ = rt;
    sched_ = &rt->scheduler;
    sched_->explicit_data_affinity_enabled = explicit_data_affinity_enabled_;
}
