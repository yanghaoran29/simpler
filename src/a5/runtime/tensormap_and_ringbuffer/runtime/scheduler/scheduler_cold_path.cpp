...
bool cluster_owned_by_thread(
    ...
    return cluster_owned_by_pthread(
        ci, tidx, cluster_count, active_threads, assignment_mode, pthread_to_logical
    );
}
...
void SchedulerContext::handshake_owned_clusters(Runtime *runtime, int32_t tidx, int32_t active_threads) {
    ...
    for (int32_t ci = 0; ci < aic_n; ++ci) {
        if (!cluster_owned_by_thread(
                ci, tidx, aic_n, active_threads, sched_aicore_assignment_mode_, sched_pthread_to_logical_
            ))
            continue;
        owned[own_n++] = ci;                  // AIC
...
void SchedulerContext::assign_own_clusters(int32_t tidx) {
    ...
    int32_t own_n = 0;
    for (int32_t ci = 0; ci < aic_n; ++ci) {
        if (!cluster_owned_by_thread(ci, tidx, aic_n, active, sched_aicore_assignment_mode_, sched_pthread_to_logical_))
            continue;
        own_n++;
    }
...
void SchedulerContext::run_die_rtt_preflight(int32_t tidx) {
    const int32_t active_threads = active_sched_threads_ > 0 ? active_sched_threads_ :
                                     (sched_thread_num_ > 0 ? sched_thread_num_ :
                                                                (aicpu_thread_num_ > 1 ? aicpu_thread_num_ - 1 :
                                                                                         aicpu_thread_num_));
