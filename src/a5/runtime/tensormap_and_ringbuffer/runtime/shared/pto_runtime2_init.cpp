...
PTO2SchedulerState::reserve_layout(DeviceArena &arena, const int32_t dep_pool_capacities[PTO2_MAX_RING_DEPTH]) {
    ...
    for (int32_t domain = 0; domain < TASK_RING_DOMAIN_COUNT; domain++) {
        for (int i = 0; i < PTO2_NUM_RESOURCE_SHAPES; i++) {
            layout.off_domain_ready_queue_slots[domain][i] =
                ready_queue_reserve_layout(arena, PTO2_READY_QUEUE_SIZE);
        }
    }
    for (int32_t thread = 0; thread < PLATFORM_MAX_AICPU_THREADS - 1; thread++) {
        for (int i = 0; i < PTO2_NUM_RESOURCE_SHAPES; i++) {
            layout.off_continuation_ready_queue_slots[thread][i] =
                ready_queue_reserve_layout(arena, PTO2_LOCAL_CONTINUATION_QUEUE_SIZE);
        }
    }
    for (int i = 0; i < PTO2_NUM_RESOURCE_SHAPES; i++) {
    ...
    for (int32_t thread = 0; thread < PLATFORM_MAX_AICPU_THREADS - 1; thread++) {
        ...
        for (int i = 0; i < PTO2_NUM_RESOURCE_SHAPES; i++) {
            if (!ready_queue_init_data_from_layout(
                    ...
                    layout.off_continuation_ready_queue_slots[thread][i], PTO2_LOCAL_CONTINUATION_QUEUE_SIZE
                )) {
                return false;
            }
        }
    ...
    for (int r = 0; r < PTO2_MAX_RING_DEPTH; r++) {
        ...
        if (orch->scheduler != nullptr) {
            orch->rings[r].task_allocator.set_reclaim_publication_request(
                orch->scheduler->publication_request_mask_for_ring(r),
                orch->scheduler->publication_ack_mask_for_ring(r)
            );
        }
        heap_offset += heap_sizes[r];
