...
| -------- | ----- | ----------- |
| `PLATFORM_MAX_BLOCKDIM` | 36 | Compile-time ceiling for blocks (each = 1 AIC + 2 AIV). This-run `N` may be lower (ACL / SKU `ai_core_cnt`); see `hardware.md` Runtime adaptation. Example: N=28 → sched 10/10/8. |
| `PLATFORM_MAX_AICPU_THREADS` | 4 | AICPU thread count (3 schedulers + 1 orchestrator) |
| `PLATFORM_MAX_AIC_PER_THREAD` | 36 | Max AIC cores per scheduler thread (ceiling) |
| `PLATFORM_MAX_AIV_PER_THREAD` | 72 | Max AIV cores per scheduler thread (ceiling) |
