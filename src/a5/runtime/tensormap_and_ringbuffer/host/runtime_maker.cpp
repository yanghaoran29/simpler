...
static void apply_env_ring_values(
    const char *name, uint64_t min_val, uint64_t max_val, bool require_power_of_2,
    uint64_t out[RUNTIME_ENV_RING_COUNT]
) {
    ...
    uint64_t parsed[RUNTIME_ENV_RING_COUNT]{};
    size_t pos = 0;
    for (int r = 0; r < RUNTIME_ENV_RING_COUNT; r++) {
...
static bool resolve_ring_config(
    ...
    for (int depth = 0; depth < RUNTIME_ENV_RING_COUNT; depth++) {
        task_window_by_depth[depth] = PTO2_TASK_WINDOW_SIZE;
        heap_by_depth[depth] = PTO2_HEAP_SIZE;
        dep_pool_by_depth[depth] = PTO2_DEP_LIST_POOL_SIZE;
    }

    apply_env_ring_values(
        "PTO2_RING_TASK_WINDOW", 4, static_cast<uint64_t>(INT32_MAX), true, task_window_by_depth
    );
    apply_env_ring_values(
        "PTO2_RING_HEAP", 1024, std::numeric_limits<uint64_t>::max(), false, heap_by_depth
    );
    apply_env_ring_values(
        "PTO2_RING_DEP_POOL", 4, static_cast<uint64_t>(INT32_MAX), false, dep_pool_by_depth
    );
