    ...
    def _run_locked(self, callable, args, config) -> None:
        ...
        try:
            ...
            try:
                ...
                try:
                    # Closing the run must not be able to strand `run_id` in the
                    # engine's registry: `_release_run` and the run-owned
                    # resource cleanup below both live under this `try`.
                    if submission_failed:
                        self._orch._fail_run_submission(
                            run_id,
                            "" if submission_error is None else _format_exc("orchestration", submission_error),
                        )
                    else:
