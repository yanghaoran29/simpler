...
AICPU + CORE_NUM      rc=3              ← restricted device-side on a5 (worked on a3;
                                          some CANN 9.2.0 builds return rc=0 val=6)
CCPU  + OCCUPY        rc=0  val=0x1     ← CCPU has 1 core, occupied
DCPU/TSCPU queries fail (rc=3) — module-level access restricted device-side
```

Earlier a5 drivers reported host-side OCCUPY as `0x1fe` while device-side
was `0x1f8`. On CANN 9.2.0 both host and device return `0x1f8`; the SMT
pair on phy_cpu_id 1 remains visible only through CPU_TOPO. So the a5
9 → 6 gap is:

- **cpu_id 0** = AICPU OS scheduler (`OS_SCHED` bit 0)
- **cpu_id 1, 2** = SMT pair on phy_cpu_id 1, AICPU-OS-reserved (present
  in CPU_TOPO — **not** PG fab-disabled — a5 does not have the
  a3-style fab-disable pattern)
- **cpu_id 3..8** = 6 user-schedulable AICPU cores

`--json` on the same SKU (2026-08-05, device 0, automatic 1O+4S) classifies
as FG with driver CPU_TOPO:
...
and the live FG dump under
[`#live-fg-topology-on-ascend950pr_9599-2026-08-05`](../../../src/a5/docs/hardware.md#live-fg-topology-on-ascend950pr_9599-2026-08-05).
...
task-submit --device auto --device-num 1 \
    --run "$REPO/tools/cann-examples/aicpu-device-query/host/build/query_device_hal \$TASK_DEVICE --platform a2a3"
```

The platform defaults to `a5`; pass `--platform a2a3` for a2/a3 devices. The
tool selects `build/lib/<platform>/dispatcher/libsimpler_aicpu_dispatcher.so`.
Set `SIMPLER_DISPATCHER_SO` only to override that path for a custom build.

On a5, pass `--json` to merge the device-side masks with Host CPU_TOPO, classify the
A5 FG/PG scenario, and print the selected affinity. Machine-readable JSON is
written to stdout. An unknown but structurally valid topology uses the runtime's
topology-ordered fallback and prints a warning to stderr.

## Running on each arch

The same CMakeLists builds for a3 and a5. Select the matching dispatcher with
`--platform`; the default is a5:
