...
- [2026-07 — A5 跨 cluster `last_task_alive` 优化计划（衍生自 PR 906）](2026-07-a5-last-task-alive-cross-cluster.md) — D ✅ 合入（`cleanup_retired` 选择性释放正确性修复，a2a3+a5，实测无回归）；**B ❌ 实测 orch 退步 ~13% → dropped**——把 orch 搬进 sched 所在 die 引入的 L3/cache 争用 > 省下的跨 die `last_task_alive` 读开销，现有「orch 跨 die 隔离」是有意为之，M5「跨 die 写是瓶颈」的前提在 A5 本机负载上不成立；**C ⚠️ 前提被 B 动摇，存疑、暂不做**。重启 B/C 需先有隔离微基准（B-0）+ full-SKU（cube≥36）跑高任务量负载
