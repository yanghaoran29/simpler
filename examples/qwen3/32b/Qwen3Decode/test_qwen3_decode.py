...
# -----------------------------------------------------------------------------------------------------------
"""Qwen3-32B single-layer decode for A2A3 tensormap_and_ringbuffer.
...
The orchestration was generated by PyPTO IR Compiler from the above model.

When ``params.dtype`` is ``bfloat16`` or ``float32``, each L2 golden compare emits
heatmaps (pass or fail) under ``outputs/golden_compare_heatmap/``; see shared
``examples/qwen3/output_compare_heatmap.py``.

Golden disk cache and stderr phase logs: ``examples/qwen3/qwen3_hooks.py`` (set
``QWEN3_SKIP_GOLDEN_DISK_CACHE=1`` to always recompute goldens).
"""
