...
# ---------------------------------------------------------------------------
# Device (on-board) run — wrap Worker.run_prepared
# ---------------------------------------------------------------------------


@contextmanager
def log_device_run(worker: Any):
    """Emit ``device_run_start`` / ``device_run_end`` around each ``run_prepared`` call."""
    orig = worker.run_prepared

    def _wrapped(callable_id: int, args=None, config=None):
        log_qwen3("device_run_start", callable_id=callable_id)
        try:
            return orig(callable_id, args, config=config)
        finally:
            log_qwen3("device_run_end", callable_id=callable_id)

    worker.run_prepared = _wrapped  # type: ignore[method-assign]
    try:
        log_qwen3("device_wrap_installed")
        yield
    finally:
        worker.run_prepared = orig  # type: ignore[method-assign]
