...
__attribute__((visibility("default"))) void build_paged_attention_graph(const ChipTaskArgs &orch_args) {
    ...
    for (uint64_t b_idx = 0; b_idx < batch; b_idx++) {
        ...
        for (uint64_t q_idx = 0; q_idx < q_loop; q_idx++) {
            ...
            // One scope is one independent batch chain. Keep its unrolled
            // groups on one Die while alternating batches across both Dies.
            PTO2_SCOPE(PTO2ScopeMode::DIE_AFFINE) {
