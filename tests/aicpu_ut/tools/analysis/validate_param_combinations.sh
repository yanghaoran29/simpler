...
# ─────────────────────────────────────────────────────────────────────────────
{
    ...
    echo "  PERF_BACKEND=0  test_linear           → test_linear_*          indices: 0 1 2"
    echo "  PERF_BACKEND=1  paged_attention        → test_paged_attention   indices: 0"
    echo "  PERF_BACKEND=2  test_batch_paged_attention → test_batch_pa_*   indices: 0 1 2"
    echo "  PERF_BACKEND=3  deg{2,4,8}_case        → test_deg_*             indices: 0 1 2"
    echo "  PERF_BACKEND=4  test_alternating_matmul_add → test_alt_*       indices: 0 1  [NEW]"
    echo "  PERF_BACKEND=5  test_benchmark_bgemm  → test_bgemm_*           indices: 0..4 [NEW]"
    echo "  PERF_BACKEND=6  test_paged_attention_unroll → test_pau_*      indices: 0 1 2 [NEW]"
    echo ""
