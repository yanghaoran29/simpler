...
PASS1_PLUGIN="file=${PLUGIN_SO},outfile=${PHASE_OUT},markers=1,marker_phases=1,marker_start=${MARKER_START},marker_end=${MARKER_END}"
...
if [[ "${INSN_TRACE_CLI}" == "1" ]] || [[ "${AICPU_UT_SUBMIT_INSN_TRACE:-0}" == "1" ]]; then
    ...
    PASS1_PLUGIN="${PASS1_PLUGIN},insn_trace=1,insn_trace_outfile=${INSN_TRACE_FILE}"
...
set +e
_qemu_log1="$(mktemp)"
"${QEMU_BIN}" -plugin "${PASS1_PLUGIN}" "${BIN}" >"${_qemu_log1}" 2>&1
_pass1_rc=$?
set -e
