...
Options:
  ...
  --insn-trace             pass#1：在 marker 窗口内逐条写出动态执行的完整反汇编（体积极大）
  --insn-trace-out <path>  原始 trace 输出路径（默认 LOG_DIR 下带时间戳）
  --help                   Show this help

Environment overrides:
  TEST_NAME, TEST_IDX, THREAD_MODE, MARKER_START, MARKER_END,
  MARKER_BUILD_GRAPH_START, MARKER_BUILD_GRAPH_END, QEMU_BIN,
  BUILD_DIR（与 run_tests.sh 一致，默认 UT_DIR/build_a2a3）,
  AICPU_UT_SUBMIT_INSN_TRACE=1（等同 --insn-trace；路径用 INSN_TRACE_FILE 或默认）
EOF
}

INSN_TRACE_CLI=0
INSN_TRACE_FILE_OVERRIDE=""
...
while [[ $# -gt 0 ]]; do
    case "$1" in
        ...
        --insn-trace) INSN_TRACE_CLI=1; shift ;;
        --insn-trace-out)
            INSN_TRACE_CLI=1
            INSN_TRACE_FILE_OVERRIDE="$2"
            shift 2 ;;
        --help|-h) usage; exit 0 ;;
...
INSN_TRACE_ENABLED=0

if [[ "${THREAD_MODE}" == "concurrent" ]]; then
    echo "[marker-count] run pass#0 (build_graph x17/x18, same binary no rebuild): ${BIN}"
    BG_PLUGIN="file=${PLUGIN_SO},outfile=${BG_OUT},markers=1,marker_start=${MARKER_BUILD_GRAPH_START},marker_end=${MARKER_BUILD_GRAPH_END}"
else
    echo "[marker-count] run pass#1 (pto2_submit_mixed_task phase insn_count): ${BIN}"
    BG_PLUGIN=""
fi
PHASE_PLUGIN="file=${PLUGIN_SO},outfile=${PHASE_OUT},markers=1,marker_phases=1,marker_start=${MARKER_START},marker_end=${MARKER_END}"
if [[ "${INSN_TRACE_CLI}" == "1" ]] || [[ "${AICPU_UT_SUBMIT_INSN_TRACE:-0}" == "1" ]]; then
    INSN_TRACE_ENABLED=1
    if [[ -n "${INSN_TRACE_FILE_OVERRIDE}" ]]; then
        INSN_TRACE_FILE="${INSN_TRACE_FILE_OVERRIDE}"
    elif [[ -n "${INSN_TRACE_FILE:-}" ]]; then
        INSN_TRACE_FILE="${INSN_TRACE_FILE}"
    else
        INSN_TRACE_FILE="${INSN_TRACE_DEFAULT}"
    fi
    PHASE_PLUGIN="${PHASE_PLUGIN},insn_trace=1,insn_trace_outfile=${INSN_TRACE_FILE}"
    echo "[marker-count] guest insn trace (pass#1) -> ${INSN_TRACE_FILE}"
fi
set +e
_qemu_log1="$(mktemp)"
if [[ "${THREAD_MODE}" == "concurrent" ]]; then
    "${QEMU_BIN}" -plugin "${BG_PLUGIN}" "${BIN}" >"${_qemu_log1}" 2>&1
    _pass0_rc=$?
    if [[ ${_pass0_rc} -ne 0 ]] || [[ ! -s "${BG_OUT}" ]]; then
        echo "[marker-count] ERROR: pass#0 build_graph failed (rc=${_pass0_rc}), missing ${BG_OUT}" >&2
        if [[ -s "${_qemu_log1}" ]]; then
            echo "[marker-count] --- QEMU build_graph log tail ---" >&2
    ...
    : >"${_qemu_log1}"
    echo "[marker-count] run pass#1 (submit phases x3/x4, same binary no rebuild): ${BIN}"
    "${QEMU_BIN}" -plugin "${PHASE_PLUGIN}" "${BIN}" >"${_qemu_log1}" 2>&1
else
    "${QEMU_BIN}" -plugin "${PHASE_PLUGIN}" "${BIN}" >"${_qemu_log1}" 2>&1
fi
_pass1_rc=$?
...
if [[ "${THREAD_MODE}" != "concurrent" ]] && [[ ! -f "${BG_OUT}" ]]; then
    echo "# (no build_graph marker in current thread-mode)" > "${BG_OUT}"
fi
...
# pass#2 需要保留 submit-args 文本输出；覆盖上游 count 模式传入的 AICPU_UT_DISABLE_PRINTF=1。
PTO2_TRACE_SUBMIT_ARGS_ENABLE=1 PTO2_INSTR_COUNT_BUILD_GRAPH_ENABLE=0 \
    AICPU_UT_DISABLE_PRINTF=0 AICPU_UT_DISABLE_ALL_LOG=0 \
    "${RUN_TESTS_SH}" "${trace_build_args[@]}"
