...
struct PTOParam {
    ...
    void add_output(Tensor& t) {
        // Use effective raw-shapes to avoid reading uninitialized raw_shapes when is_raw_eq_shapes=true.
        arg_.add_output(TensorCreateInfo(t.get_raw_shapes(), t.ndims, t.dtype, t.manual_dep));
    ...
    void _apply_outputs(const TaskOutputTensors& result) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            output_ptrs_[i]->buffer = result.output_ptr(i)->buffer;
        }
    }
};
