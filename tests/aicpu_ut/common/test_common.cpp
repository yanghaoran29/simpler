...
#endif
            task_id = rt->scheduler.get_ready_task(static_cast<PTO2ResourceShape>(rs));
            if (task_id < 0) break;
#if PTO2_PROFILING
            uint64_t t1 = get_sys_cnt_aicpu();
            g_sched_prof_data.dispatch_cycle += t1 - t0;
#endif
            int32_t wmask_ = rt->sm_handle->header->task_window_size - 1;
            uint8_t amask = rt->sm_handle->task_descriptors[task_id & wmask_].active_mask;
            if (amask & PTO2_SUBTASK_MASK_AIC)  rt->scheduler.on_subtask_complete(task_id, PTO2SubtaskSlot::AIC);
            if (amask & PTO2_SUBTASK_MASK_AIV0) rt->scheduler.on_subtask_complete(task_id, PTO2SubtaskSlot::AIV0);
            if (amask & PTO2_SUBTASK_MASK_AIV1) rt->scheduler.on_subtask_complete(task_id, PTO2SubtaskSlot::AIV1);
#if PTO2_SCHED_PROFILING
            // In simulation mode, use thread_idx=0 since we're single-threaded
            PTO2CompletionStats cs = rt->scheduler.on_mixed_task_complete(task_id, 0);
#elif PTO2_PROFILING
            PTO2CompletionStats cs = rt->scheduler.on_mixed_task_complete(task_id);
#else
            rt->scheduler.on_mixed_task_complete(task_id);
            PTO2CompletionStats cs = {0, 0, 0, false};
            (void)cs;
#endif
            // Mirror device: after completion, release fanin producers so they can become CONSUMED.
#if PTO2_SCHED_PROFILING
            int32_t fe = rt->scheduler.on_task_release(task_id, 0);
#else
            int32_t fe = rt->scheduler.on_task_release(task_id);
#endif
#if PTO2_PROFILING
    ...
    while (idle_rounds < max_iterations) {
        ...
        for (int rs = 0; rs < PTO2_NUM_RESOURCE_SHAPES; ++rs) {
            ...
            while (true) {
                ...
                if (amask & PTO2_SUBTASK_MASK_AIV1) rt->scheduler.on_subtask_complete(task_id, PTO2SubtaskSlot::AIV1);
#if PTO2_SCHED_PROFILING
                PTO2CompletionStats cs = rt->scheduler.on_mixed_task_complete(task_id, thread_idx);
#elif PTO2_PROFILING
                PTO2CompletionStats cs = rt->scheduler.on_mixed_task_complete(task_id);
#else
                rt->scheduler.on_mixed_task_complete(task_id);
                ...
                // Mirror device: after completion, release fanin producers so they can become CONSUMED.
#if PTO2_SCHED_PROFILING
                int32_t fe = rt->scheduler.on_task_release(task_id, thread_idx);
#else
                int32_t fe = rt->scheduler.on_task_release(task_id);
#endif
#if PTO2_PROFILING
