...
PTO2Runtime* make_runtime() {
    // main removed pto2_runtime_create_custom; replicate its arena-based runtime
    // creation here for the in-process simulator (host == device).
    constexpr uint64_t task_window_size = 262144;
    constexpr uint64_t heap_size = 4ull * 1024 * 1024 * 1024;  // per-ring

    // The arena backs the PTO2Runtime image; it must outlive rt. The UT process
    // is short-lived, so we intentionally leak it (one per make_runtime call).
    DeviceArena* arena = new DeviceArena();
    PTO2RuntimeArenaLayout layout = runtime_reserve_layout(*arena, task_window_size);
    ...
    const uint64_t total_heap = heap_size * PTO2_MAX_RING_DEPTH;
    void* sm_ptr = nullptr;
    void* gm_heap = nullptr;
    if (posix_memalign(&sm_ptr, PTO2_ALIGN_SIZE, sm_size) != 0) return nullptr;
    if (posix_memalign(&gm_heap, PTO2_ALIGN_SIZE, total_heap) != 0) return nullptr;

    PTO2Runtime* rt =
        runtime_init_data_from_layout(*arena, layout, PTO2_MODE_SIMULATE, sm_ptr, sm_size, gm_heap, heap_size);
    if (!rt) return nullptr;
    runtime_wire_arena_pointers(*arena, layout, rt);

    // Device-side finalize, mirroring aicpu_executor.cpp's boot: reset SM and the
    // AICore completion mailbox, then fill ops + orchestrator cluster counts.
    ...
    const int32_t aic_count = PLATFORM_MAX_CORES / 3;
    runtime_finalize_after_wire(rt, aic_count, PLATFORM_MAX_CORES - aic_count);
...
int sim_drain_one_pass(PTO2Runtime* rt) {
    ...
    for (int rs = 0; rs < PTO2_NUM_RESOURCE_SHAPES; ++rs) {
        while (true) {
            ...
            PTO2TaskSlotState* ready_slots[1] = {nullptr};
            int ready_cnt = rt->scheduler.get_ready_tasks_batch(
                static_cast<PTO2ResourceShape>(rs), local_buf, ready_slots, 1);
            PTO2TaskSlotState* slot_state = (ready_cnt > 0) ? ready_slots[0] : nullptr;
            if (!slot_state) break;
#if PTO2_PROFILING
            uint64_t t1 = get_sys_cnt_aicpu();
            g_sched_prof_data.dispatch_cycle += t1 - t0;
#endif
            auto amask = slot_state->active_mask;
            if (amask & PTO2_SUBTASK_MASK_AIC)  rt->scheduler.on_subtask_complete(*slot_state);
            if (amask & PTO2_SUBTASK_MASK_AIV0) rt->scheduler.on_subtask_complete(*slot_state);
            if (amask & PTO2_SUBTASK_MASK_AIV1) rt->scheduler.on_subtask_complete(*slot_state);
#if PTO2_SCHED_PROFILING
            // In simulation mode, use thread_idx=0 since we're single-threaded
            CompletionStats cs = rt->scheduler.on_mixed_task_complete(*slot_state, 0, nullptr);
#elif PTO2_PROFILING
            rt->scheduler.on_mixed_task_complete(*slot_state, nullptr);
            CompletionStats cs = {0, 0, 0, false};
#else
            rt->scheduler.on_mixed_task_complete(*slot_state, nullptr);
            CompletionStats cs = {0, 0, 0, false};
            (void)cs;
...
void run_sched_checks(PTO2Runtime* rt, int num_sched) {
    ...
    for (int ri = 0; ri < PTO2_MAX_RING_DEPTH; ri++) {
        ...
        for (int32_t local_id = 0; local_id < current; local_id++) {
            const PTO2TaskSlotState& slot = rt->sm_handle->header->rings[ri].get_slot_state_by_task_id(local_id);
            int16_t bn = slot.logical_block_num;
