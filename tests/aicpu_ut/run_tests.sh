...
#   ./run_tests.sh --skip-pip-install              # 跳过默认的「在 simpler 目录下 pip install .」
...
#   test_qwen3                      (perf, qwen3 prefill layer; indices 0=full 1=light; --orch/--sched; 支持 --arch a5)
#
# Optional environment overrides:
#   TIMEOUT=300 ./run_tests.sh
#   AICPU_UT_SKIP_PIP_INSTALL=1 ./run_tests.sh    # 不执行默认 pip install（等同 --skip-pip-install）
...
TEST_TYPE["test_qwen3"]="perf"                  ; TEST_INDICES["test_qwen3"]="0 1"
...
SKIP_PIP_INSTALL=false          # set by --skip-pip-install; or env AICPU_UT_SKIP_PIP_INSTALL=1
...
run_self_check() {
    ...
    for cmd in "${checks[@]}"; do
        ...
        tmp_log="$(mktemp)"
        set +e
        env AICPU_UT_SKIP_PIP_INSTALL=1 bash "$self" ${cmd} 2>&1 | tee "$tmp_log"
        local rc=$?
        set -e
...
# 默认在仓库的 simpler 包根目录执行 pip install .（可用 --skip-pip-install 或 AICPU_UT_SKIP_PIP_INSTALL=1 跳过）。
# 父进程在 profiling re-exec 前若已安装，会通过 AICPU_UT_RAN_PIP_FOR_RUN_TESTS 避免子进程重复安装。
maybe_pip_install_simpler() {
    if [ "$SKIP_PIP_INSTALL" = true ]; then
        return 0
    fi
    if [ "${AICPU_UT_SKIP_PIP_INSTALL:-0}" = 1 ]; then
        return 0
    fi
    if [ -n "${AICPU_UT_RAN_PIP_FOR_RUN_TESTS:-}" ]; then
        return 0
    fi
    if [ ! -f "${SIMPLER_ROOT}/pyproject.toml" ] && [ ! -f "${SIMPLER_ROOT}/setup.py" ]; then
        return 0
    fi
    (cd "$SIMPLER_ROOT" && pip install .) || return 1
    export AICPU_UT_RAN_PIP_FOR_RUN_TESTS=1
    return 0
}
...
if [ "${COUNT_SCHEDULER_LOOP_INSNS:-0}" = 1 ]; then
    ...
    env BUILD_DIR="$BUILD_DIR" ARCH="$ARCH" "$sched_tool" --test "$test_arg" --idx "$idx_arg" --thread-mode "$mode_arg"
    rc=$?
    if [ $rc -ne 0 ]; then
        exit $rc
    fi
    ...
    if [ -x "$sched_insn_script" ]; then
        env BUILD_DIR="$BUILD_DIR" ARCH="$ARCH" "$sched_insn_script" --test "$test_arg" --idx "$idx_arg" --thread-mode "$mode_arg" || true
    fi
...
if [ "${COUNT_SUBMIT_TASK_INSNS:-0}" = 1 ]; then
    ...
    test_arg="${FILTER_TEST:-test_batch_paged_attention}"
    idx_arg="${FILTER_IDX:-0}"
    _marker_tool_env=(AICPU_UT_DISABLE_PRINTF=1 BUILD_DIR="$BUILD_DIR" ARCH="$ARCH")
    ...
    if [ -x "$insn_script" ]; then
        env AICPU_UT_DISABLE_PRINTF=1 ARCH="$ARCH" BUILD_DIR="$BUILD_DIR" \
...
# ─── Helpers ──────────────────────────────────────────────────────────────────
# When AICPU_UT_QUIET=1 (profiling off), skip banners/separators; final summary still prints via plain echo.
...
run_binary() {
    ...
    if [ -n "${AICPU_UT_QUIET:-}" ]; then
        # profiling off: hide test binary stdout (less spam); keep stderr so progress logs
        # and errors are still visible. Ctrl+C still goes to the
        # foreground group (timeout + child); if unresponsive, try SIGTERM/SIGKILL from
        # another terminal: pkill -f test_qwen3_concurrent
        if timeout "$TIMEOUT" "$bin" "$@" >/dev/null; then
            return 0
        else
            return $?
        fi
...
# ─── Summary ──────────────────────────────────────────────────────────────────
# Always print summary (even when AICPU_UT_QUIET=1) so default --profiling 0 runs are not silent.
echo "============================================================"
