...
inline int export_sim_swimlane(PTO2Runtime* rt, const char* output_dir = nullptr) {
    ...
    struct TaskInfo {
        ...
        int32_t worker_type;
        int32_t fanin_count;
        int32_t fanin_tasks[PTO2_FANIN_INLINE_CAP];
    };
    ...
    for (int32_t tid = 0; tid < total_tasks; tid++) {
        ...
        const PTO2TaskDescriptor& desc    = sm->header->rings[0].get_task_by_slot(slot);
        const PTO2TaskPayload&    payload = sm->header->rings[0].get_payload_by_slot(slot);
        PTO2TaskSlotState&        slot_state = sm->header->rings[0].get_slot_state_by_slot(slot);
        ...
        { int32_t kid = 0;
          ...
          info.kernel_id = kid; }
        info.worker_type   = (slot_state.active_mask & PTO2_SUBTASK_MASK_AIC)
                                 ? PTO2_WORKER_CUBE : PTO2_WORKER_VECTOR;
        info.fanin_count   = payload.fanin_actual_count;
        int n = (payload.fanin_actual_count < PTO2_FANIN_INLINE_CAP)
                    ? payload.fanin_actual_count : PTO2_FANIN_INLINE_CAP;
        for (int k = 0; k < n; k++)
            info.fanin_tasks[k] = payload.fanin_inline_slot_states[k] != nullptr
                ? static_cast<int32_t>(payload.fanin_inline_slot_states[k]->task->task_id.local()) : -1;
    }
