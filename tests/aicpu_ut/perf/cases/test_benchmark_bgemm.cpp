...
void build_graph(PTO2Runtime* rt, uint64_t* args, int arg_count) {
    ...
    uint32_t A_shapes[1] = {static_cast<uint32_t>(size_A / sizeof(float))};
    uint32_t B_shapes[1] = {static_cast<uint32_t>(size_B / sizeof(float))};
    uint32_t C_shapes[1] = {static_cast<uint32_t>(size_C / sizeof(float))};
    ...
    uint32_t config_shapes[1] = {static_cast<uint32_t>(BGEMM_CONFIG_NELEMS)};
    ...
    for (int group_idx = 0; group_idx < num_groups; group_idx++) {
        PTO2_SCOPE(rt) {
            ...
            uint32_t c_offsets[1]  = {static_cast<uint32_t>(c_offset)};
