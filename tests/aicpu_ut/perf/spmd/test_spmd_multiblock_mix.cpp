 ...
 *
 * Case 1 — block_num=1, repeated 1024 times (many fine-grained MIX submits):
 *   1024 × (1 block × 3 CL) = 3072 CL = 49152 float32.
 *
 * Case 2 — block_num=8, repeated 128 times (same total work as case 1):
 *   128 × (8 blocks × 3 CL) = 3072 CL = 49152 float32.
 *
 * Case 1/2: do not pass the full output tensor per task. A parent tensor covers the
...
static constexpr int SPMD_MIX_CASE_COUNT = 3;
...
// 1024 tasks × 1 block × 3 CL = 3072 CL
static constexpr size_t SPMD_MIX_TOTAL_CL     = 3072;
#elif PERF_CASE_IDX == 2
// 128 tasks × 8 blocks × 3 CL = 3072 CL
static constexpr size_t SPMD_MIX_TOTAL_CL     = 3072;
#endif
...
const SpmdMultiblockMixTestCase PERF_CASES[SPMD_MIX_CASE_COUNT] = {
    ...
    { "SPMD Multi-Block MIX case1: block_num=1 × 1024 tasks (3072 CL)" },
    { "SPMD Multi-Block MIX case2: block_num=8 × 128 tasks (3072 CL)" },
};
...
void build_graph(PTO2Runtime* rt, uint64_t* args, int arg_count) {
    ...
    // Parent buffer descriptor; case1/case2 submit only per-task views (constant slice size).
    ...
    PTO2_SCOPE(rt) {
        ...
        total_tasks++;
#elif PERF_CASE_IDX == 1
        // Constant slice: 1 block × 3 CL; one view per task (not full tensor).
        constexpr uint32_t k_floats_per_task =
            static_cast<uint32_t>(SPMD_SLOTS_PER_BLOCK * SPMD_M_FLOATS_PER_CL);
...
#elif PERF_CASE_IDX == 1
    printf("  Output: %zu floats (%zu cache lines): 1024×MIX block_num=1; "
           "constant 3-CL views + manual_dep (flat TensorMap lookup cost)\n",
           SPMD_MIX_TOTAL_FLOATS, SPMD_MIX_TOTAL_CL);
#elif PERF_CASE_IDX == 2
    printf("  Output: %zu floats (%zu cache lines): 128×MIX block_num=8; "
