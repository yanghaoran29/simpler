 ...
 * Minimal dependency graph: chain_length=1 (single task from input to output)
 * to measure end-to-end latency of submit → schedule → complete.
 *
...
struct LatencyTestCase {
    const char* name;
    int         tensor_nelems; // elements per tensor (small for min latency)
};

static constexpr int LATENCY_CASE_COUNT = 1;
static_assert(PERF_CASE_IDX >= 0 && PERF_CASE_IDX < LATENCY_CASE_COUNT,
              "PERF_CASE_IDX out of range");

extern const LatencyTestCase PERF_CASES[LATENCY_CASE_COUNT];

static constexpr size_t LATENCY_MAX_NELEMS = 64;

extern float g_latency_input_buf[LATENCY_MAX_NELEMS];
extern float g_latency_output_buf[LATENCY_MAX_NELEMS];
...
const LatencyTestCase PERF_CASES[LATENCY_CASE_COUNT] = {
    { "MinLatency (chain=1, tensor=64 floats)", 64 },
};

float g_latency_input_buf[LATENCY_MAX_NELEMS];
float g_latency_output_buf[LATENCY_MAX_NELEMS];
...
void build_latency_graph(PTO2Runtime* rt, uint64_t* args, int arg_count) {
    ...
    int tensor_nelems = static_cast<int>(config[0]);

    printf("  chain_length = 1, tensor_nelems = %d\n", tensor_nelems);

    DataType dtype   = DataType::FLOAT32;
    uint64_t shape[1] = {static_cast<uint64_t>(tensor_nelems)};

    Tensor in  = make_tensor_external(input_buf, shape, 1, dtype);
    Tensor out = make_tensor_external(output_buf, shape, 1, dtype);
    ...
    printf("  Total tasks submitted: 1\n");
}

PTO2Runtime* setup_run(const LatencyTestCase& tc, LatencyRunCtx& ctx) {
    ctx.config[0] = static_cast<int64_t>(tc.tensor_nelems);
    ctx.config[1] = 0;
    ctx.config[2] = 0;
    ctx.config[3] = 0;
...
void print_config(const LatencyTestCase& tc) {
    ...
    printf("  chain_length = 1, tensor_nelems = %d (min latency: single task)\n",
           tc.tensor_nelems);
}
