...
extern float g_query_buf[PA_QUERY_NELEMS];
extern float g_key_cache_buf[PA_KV_NELEMS];
extern float g_value_cache_buf[PA_KV_NELEMS];
extern float g_out_buf[PA_QUERY_NELEMS];
extern int   g_block_table[PA_BLOCK_TABLE_CNT];
extern int   g_context_lens[PA_CONTEXT_LENS_CNT];
    ...
    for (uint64_t b_idx = 0; b_idx < batch; b_idx++) {
        ...
        for (uint64_t q_idx = 0; q_idx < q_loop; q_idx++) {
            PTO2_SCOPE(rt) {
                ...
                uint64_t out_v_shapes[2]   = {q_tile, head_dim};
                uint64_t out_v_offsets[2]  = {cur_offset, 0};
