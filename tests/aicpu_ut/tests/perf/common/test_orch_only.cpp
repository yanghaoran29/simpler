/**
 * test_orch_only.cpp
 *
 * Orchestration only (no scheduler) — one binary per (backend, case). Backend
 * selected at compile time via PERF_BACKEND (0=linear, 1=pa, 2=batch_pa, 3=degree,
 * 4=alternating, 5=bgemm, 6=pa_unroll).
 *
 * Compile: -DPERF_BACKEND=N -DPERF_CASE_IDX=M
 */

#if PERF_BACKEND == 0
#include "linear_case.cpp"
#elif PERF_BACKEND == 1
#include "paged_attention.cpp"
#elif PERF_BACKEND == 2
#include "batch_paged_attention.cpp"
#elif PERF_BACKEND == 4
#include "alternating_matmul_add.cpp"
#elif PERF_BACKEND == 5
#include "benchmark_bgemm.cpp"
#elif PERF_BACKEND == 6
#include "paged_attention_unroll.cpp"
#elif PERF_BACKEND == 7
#include "throughput_case.cpp"
#elif PERF_BACKEND == 8
#include "latency_case.cpp"
#elif PERF_CASE_IDX == 1
#include "deg4_case.cpp"
#elif PERF_CASE_IDX == 2
#include "deg8_case.cpp"
#else
#include "deg2_case.cpp"
#endif

int g_pass = 0;
int g_fail = 0;

int main() {
#if PTO2_PROFILING
    setvbuf(stdout, nullptr, _IOLBF, 0);  // line-buffered so profiling prints appear when piped
#if PERF_BACKEND == 0
    printf("\n============================================================\n");
    printf("  Linear Chain — Orchestration Only (No Scheduler)\n");
    printf("============================================================\n\n");
#elif PERF_BACKEND == 1
    printf("\n============================================================\n");
    printf("  Paged Attention — Orchestration Only (No Scheduler)\n");
    printf("============================================================\n\n");
#elif PERF_BACKEND == 2
    printf("\n============================================================\n");
    printf("  Batch Paged Attention — Orchestration Only (No Scheduler)\n");
    printf("============================================================\n\n");
#elif PERF_BACKEND == 4
    printf("\n============================================================\n");
    printf("  Alternating Matmul+Add — Orchestration Only (No Scheduler)\n");
    printf("============================================================\n\n");
#elif PERF_BACKEND == 5
    printf("\n============================================================\n");
    printf("  Batched GEMM — Orchestration Only (No Scheduler)\n");
    printf("============================================================\n\n");
#elif PERF_BACKEND == 6
    printf("\n============================================================\n");
    printf("  Paged Attention Unroll — Orchestration Only (No Scheduler)\n");
    printf("============================================================\n\n");
#elif PERF_BACKEND == 7
    printf("\n============================================================\n");
    printf("  Max Throughput — Orchestration Only (No Scheduler)\n");
    printf("============================================================\n\n");
#elif PERF_BACKEND == 8
    printf("\n============================================================\n");
    printf("  Min Latency — Orchestration Only (No Scheduler)\n");
    printf("============================================================\n\n");
#else
    printf("\n============================================================\n");
    printf("  Degree DAG — Orchestration Only (No Scheduler)\n");
    printf("============================================================\n\n");
#endif
#endif

#if PERF_BACKEND == 0
    const LinearTestCase& tc = PERF_CASES[PERF_CASE_IDX];
    bind_to_cpu(ORCH_CPU);
#if PTO2_PROFILING
    print_config(tc);
    section_header_100('-', "--- CPU affinity ---");
    int orch_core = current_cpu();
    printf("  orchestrator → core %d\n", orch_core >= 0 ? orch_core : ORCH_CPU);
    printf("\n");
    section_header_100('-', "--- Orchestrator Profiling ---");
#endif
    LinearRunCtx ctx;
    PTO2Runtime* rt = setup_run(tc, ctx);
    if (!rt) return 1;
    perf_wait_sigstop();
    build_linear_graph(rt, ctx.args, 10);
#elif PERF_BACKEND == 1
    const PerfTestCase& tc = PERF_CASES[PERF_CASE_IDX];
    bind_to_cpu(ORCH_CPU);
#if PTO2_PROFILING
    print_config(tc);
    section_header_100('-', "--- CPU affinity ---");
    int orch_core = current_cpu();
    printf("  orchestrator → core %d\n", orch_core >= 0 ? orch_core : ORCH_CPU);
    printf("\n");
    section_header_100('-', "--- Orchestrator Profiling ---");
#endif
    PARunCtx ctx;
    PTO2Runtime* rt = setup_run(tc, ctx);
    if (!rt) return 1;
    perf_wait_sigstop();
    build_paged_attention_graph(rt, ctx.args, 10);
#elif PERF_BACKEND == 2
    const PerfTestCase& tc = PERF_CASES[PERF_CASE_IDX];
    bind_to_cpu(ORCH_CPU);
#if PTO2_PROFILING
    print_config(tc);
    section_header_100('-', "--- CPU affinity ---");
    int orch_core = current_cpu();
    printf("  orchestrator → core %d\n", orch_core >= 0 ? orch_core : ORCH_CPU);
    printf("\n");
    section_header_100('-', "--- Orchestrator Profiling ---");
#endif
    BatchPARunCtx ctx;
    PTO2Runtime* rt = setup_run(tc, ctx);
    if (!rt) return 1;
    perf_wait_sigstop();
    build_batch_paged_attention_graph(rt, ctx.args, 10);
#elif PERF_BACKEND == 4
    const AlternatingTestCase& tc = PERF_CASES[PERF_CASE_IDX];
    bind_to_cpu(ORCH_CPU);
#if PTO2_PROFILING
    print_config(tc);
    section_header_100('-', "--- CPU affinity ---");
    int orch_core = current_cpu();
    printf("  orchestrator → core %d\n", orch_core >= 0 ? orch_core : ORCH_CPU);
    printf("\n");
    section_header_100('-', "--- Orchestrator Profiling ---");
#endif
    AlternatingRunCtx ctx;
    PTO2Runtime* rt = setup_run(tc, ctx);
    if (!rt) return 1;
    perf_wait_sigstop();
    build_alternating_graph(rt, ctx.args, 10);
#elif PERF_BACKEND == 5
    const BgemmTestCase& tc = PERF_CASES[PERF_CASE_IDX];
    bind_to_cpu(ORCH_CPU);
#if PTO2_PROFILING
    print_config(tc);
    section_header_100('-', "--- CPU affinity ---");
    int orch_core = current_cpu();
    printf("  orchestrator → core %d\n", orch_core >= 0 ? orch_core : ORCH_CPU);
    printf("\n");
    section_header_100('-', "--- Orchestrator Profiling ---");
#endif
    BgemmRunCtx ctx;
    PTO2Runtime* rt = setup_run(tc, ctx);
    if (!rt) return 1;
    perf_wait_sigstop();
    build_bgemm_graph(rt, ctx.args, 10);
#elif PERF_BACKEND == 6
    const PerfTestCase& tc = PERF_CASES[PERF_CASE_IDX];
    bind_to_cpu(ORCH_CPU);
#if PTO2_PROFILING
    print_config(tc);
    section_header_100('-', "--- CPU affinity ---");
    int orch_core = current_cpu();
    printf("  orchestrator → core %d\n", orch_core >= 0 ? orch_core : ORCH_CPU);
    printf("\n");
    section_header_100('-', "--- Orchestrator Profiling ---");
#endif
    PAUnrollRunCtx ctx;
    PTO2Runtime* rt = setup_run(tc, ctx);
    if (!rt) return 1;
    perf_wait_sigstop();
    build_paged_attention_unroll_graph(rt, ctx.args, 10);
#elif PERF_BACKEND == 7
    const ThroughputTestCase& tc = PERF_CASES[PERF_CASE_IDX];
    bind_to_cpu(ORCH_CPU);
#if PTO2_PROFILING
    print_config(tc);
    section_header_100('-', "--- CPU affinity ---");
    int orch_core = current_cpu();
    printf("  orchestrator → core %d\n", orch_core >= 0 ? orch_core : ORCH_CPU);
    printf("\n");
    section_header_100('-', "--- Orchestrator Profiling ---");
#endif
    ThroughputRunCtx ctx;
    PTO2Runtime* rt = setup_run(tc, ctx);
    if (!rt) return 1;
    perf_wait_sigstop();
    build_throughput_graph(rt, ctx.args, 10);
#elif PERF_BACKEND == 8
    const LatencyTestCase& tc = PERF_CASES[PERF_CASE_IDX];
    bind_to_cpu(ORCH_CPU);
#if PTO2_PROFILING
    print_config(tc);
    section_header_100('-', "--- CPU affinity ---");
    int orch_core = current_cpu();
    printf("  orchestrator → core %d\n", orch_core >= 0 ? orch_core : ORCH_CPU);
    printf("\n");
    section_header_100('-', "--- Orchestrator Profiling ---");
#endif
    LatencyRunCtx ctx;
    PTO2Runtime* rt = setup_run(tc, ctx);
    if (!rt) return 1;
    perf_wait_sigstop();
    build_latency_graph(rt, ctx.args, 10);
#else
    const DegreeTestCase& tc = PERF_CASES[0];
    bind_to_cpu(ORCH_CPU);
#if PTO2_PROFILING
    print_config(tc);
    section_header_100('-', "--- CPU affinity ---");
    int orch_core = current_cpu();
    printf("  orchestrator → core %d\n", orch_core >= 0 ? orch_core : ORCH_CPU);
    printf("\n");
    section_header_100('-', "--- Orchestrator Profiling ---");
#endif
    DegRunCtx ctx;
    PTO2Runtime* rt = setup_run(tc, ctx);
    if (!rt) return 1;
    perf_wait_sigstop();
    build_degree_graph(rt, ctx.args, 10);
#endif

#if PTO2_PROFILING
    print_orch_profiling();
    if (!getenv("AICPU_UT_NO_CHECK")) {
        int32_t submitted = 0;
        if (rt->sm_handle && rt->sm_handle->header)
            submitted = rt->sm_handle->header->current_task_index.load(std::memory_order_acquire);
        if (submitted == 0) {
            printf("  FAIL (P1): orchestration submitted 0 tasks\n");
            g_fail++;
        } else {
            g_pass++;
        }
    }
    printf("  Orchestration-only run finished (no scheduler threads).\n");
#endif

    pto2_runtime_destroy(rt);
    return (g_fail == 0) ? 0 : 1;
}
