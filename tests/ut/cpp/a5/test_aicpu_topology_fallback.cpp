...
TEST(A5AicpuTopologyFallback, EnumeratesEveryOccupiedCpu) {
    ...
    ASSERT_TRUE(enumerate_cpus_from_occupy((1ULL << 1) | (1ULL << 3) | (1ULL << 5), cpus));
    ASSERT_EQ(cpus.size(), 3U);

    for (int32_t i = 0; i < 3; ++i) {
        EXPECT_EQ(cpus[i].cpu_id, 2 * i + 1);
...
TEST(A5AicpuTopologyFallback, WorksForAnyNonEmptyOccupyMask) {
    std::vector<AicpuLogicalCpu> cpus;
    ASSERT_TRUE(enumerate_cpus_from_occupy((1ULL << 0) | (1ULL << 17) | (1ULL << 63), cpus));
    ASSERT_EQ(cpus.size(), 3U);
    EXPECT_EQ(cpus[0].cpu_id, 0);
    EXPECT_EQ(cpus[1].cpu_id, 17);
    EXPECT_EQ(cpus[2].cpu_id, 63);
}
...
TEST(A5AicpuTopologyFallback, SelectsFiveCpuIdsWithoutTopologyRequirements) {
    AicpuTopology topology;
    ASSERT_TRUE(enumerate_cpus_from_occupy(0x3fU, topology.os_schedulable_cpus));
    ...
    EXPECT_EQ(allowed, (std::vector<int32_t>{0, 1, 2, 3, 4}));
}
...
TEST(A5AicpuTopologyJsonFallback, RejectsUnexpected9599OccupyMask) {
    std::vector<AicpuLogicalCpu> cpus = {{1, 1, 0, 0, 0}};

    EXPECT_FALSE(load_cpu_topo_from_json("Ascend950PR_9599", 0x1f0, cpus));
    EXPECT_TRUE(cpus.empty());
}
...
TEST(A5AicpuTopologyJsonFallback, RejectsUnexpectedVerifiedFallbackOccupyMask) {
    std::vector<AicpuLogicalCpu> cpus = {{1, 1, 0, 0, 0}};
    EXPECT_FALSE(load_cpu_topo_from_json("Ascend950PR_9579", 0x1f8, cpus));
    EXPECT_TRUE(cpus.empty());
}

TEST(A5AicpuTopologyJsonFallback, RejectsUnknownAndMissingSoc) {
    std::vector<AicpuLogicalCpu> cpus = {{1, 1, 0, 0, 0}};

    EXPECT_FALSE(load_cpu_topo_from_json("Ascend950PR_unknown", 0, cpus));
    EXPECT_TRUE(cpus.empty());

    cpus = {{1, 1, 0, 0, 0}};
    EXPECT_FALSE(load_cpu_topo_from_json("Ascend950PR_missing", 0, cpus));
    EXPECT_TRUE(cpus.empty());

    EXPECT_FALSE(load_cpu_topo_from_json(nullptr, 0, cpus));
    EXPECT_TRUE(cpus.empty());
}
...
TEST(A5AicpuTopologySelection, ClassifiesKnownScenarios) {
    ...
    // Logical count is not a gate: 9-CPU layout with 4 clusters is still FG.
    EXPECT_EQ(
        classify_aicpu_scenario(static_cast<uint32_t>(fg_all.size()), fg_all, primaries(fg_all)), AicpuScenarioType::kFg
    );
    ...
    // and by classifying with the vector's actual size above.
...
TEST(A5AicpuLaunchPlan, WarnsWhenCpuTopologyFallsBackToOccupy) {
    AicpuTopology topology;
    topology.source = AicpuTopologySource::kOccupyFallback;
    topology.scenario_type = AicpuScenarioType::kUnknown;
    ASSERT_TRUE(enumerate_cpus_from_occupy(0x1fU, topology.os_schedulable_cpus));
    AicpuLaunchPlan plan;
    std::string error;
...
TEST(A5AicpuTopologySelection, UnknownFallbackRequiresSchedulerAndOrchestrator) {
    AicpuTopology topology;
...
TEST(A5AicpuTopologySelection, UnknownFallbackRejectsInvalidOrInsufficientRequestedCount) {
    AicpuTopology topology;
...
TEST(A5AicpuTopologySelection, JsonIncludesFallbackPolicyAndAffinity) {
    AicpuTopology topology;
    topology.soc_name = "Ascend950PR_\"test\"\\control\n";
    topology.source = AicpuTopologySource::kOccupyFallback;
    topology.scenario_type = AicpuScenarioType::kUnknown;
    topology.logical_cpu_count = 6;
    topology.device_occupancy.occupy = 0x15;
    topology.os_schedulable_cpus = {{1, 1, 0, 0, 0}, {2, 2, 0, 1, 0}};
    const std::string json = format_aicpu_topology_json(topology, AicpuSelectionPolicy::kTopologyFallback, {1, 2});
    EXPECT_NE(json.find("\"soc_name\": \"Ascend950PR_\\\"test\\\"\\\\control\\u000a\""), std::string::npos);
    EXPECT_NE(json.find("\"topology_source\": \"occupy_fallback\""), std::string::npos);
    EXPECT_NE(json.find("\"selection_policy\": \"topology_fallback\""), std::string::npos);
