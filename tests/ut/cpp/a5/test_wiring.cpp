...
protected:
    ...
    void request_and_drain_publication(int32_t ring_id = 0) {
        sched.publication_request_mask_for_ring(ring_id)->store(
            ring_mask_bit(ring_id), std::memory_order_release
        );
...
TEST_F(WiringTest, NoFaninTaskBecomesReady) {
    ...
    auto *popped = sched.domain_ready_queues[static_cast<int32_t>(TaskReadyDomain::GLOBAL)]
                                                  [static_cast<int32_t>(shape)]
                                                      .pop();
...
TEST_F(WiringTest, OnMixedTaskCompleteNotifiesConsumers) {
    ...
    auto *r1 = sched.domain_ready_queues[static_cast<int32_t>(TaskReadyDomain::GLOBAL)]
                                              [static_cast<int32_t>(shape)]
                                                  .pop();
    auto *r2 = sched.domain_ready_queues[static_cast<int32_t>(TaskReadyDomain::GLOBAL)]
                                              [static_cast<int32_t>(shape)]
                                                  .pop();
...
TEST_F(WiringTest, DeferredReleaseBatchCombinesSharedProducerReferences) {
    alignas(64) PTO2TaskSlotState producer;
    alignas(64) PTO2TaskSlotState consumers[2];
    PTO2FaninPool spill_pool{};
    PTO2FaninSpillEntry spill_entries[4];
    std::atomic<int32_t> spill_error{PTO2_ERROR_NONE};
    ...
    init_slot(producer, PTO2_TASK_COMPLETED, 0, 2);
    for (PTO2TaskSlotState &consumer : consumers) {
        init_slot(consumer, PTO2_TASK_COMPLETED, 1, 1);
    ...
    PTO2TaskSlotState *deferred[] = {&consumers[0], &consumers[1]};
    int32_t deferred_count = 2;
    EXPECT_EQ(
        sched.release_deferred_batch(
            deferred, deferred_count, /*max_batch=*/16, /*thread_idx=*/0
        ),
        2
    );
    ...
    EXPECT_EQ(producer.task_state.load(std::memory_order_acquire), PTO2_TASK_CONSUMED);
}
...
TEST_F(WiringTest, EarlyDispatchQueueOverflowFallsBackToNormalDispatch) {
    ...
    EXPECT_EQ(
        sched.domain_ready_queues[static_cast<int32_t>(TaskReadyDomain::GLOBAL)]
                                 [static_cast<int32_t>(shape)]
                                     .pop(),
        &consumer
    );
