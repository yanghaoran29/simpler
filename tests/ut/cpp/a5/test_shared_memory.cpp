...
TEST(SharedMemoryCalcSize, PerRingDifferentSizes) {
    uint64_t ws[PTO2_MAX_RING_DEPTH] = {128, 256, 512, 1024, 128, 256, 512, 1024, 128, 256, 512, 1024};
...
TEST(SharedMemoryLayout, InitPerRingWritesHeaderValues) {
    uint64_t ws[PTO2_MAX_RING_DEPTH] = {16, 32, 64, 128, 16, 32, 64, 128, 16, 32, 64, 128};
    uint64_t heaps[PTO2_MAX_RING_DEPTH] = {
        10 * 1024, 20 * 1024, 30 * 1024, 40 * 1024, 10 * 1024, 20 * 1024,
...
TEST(RuntimeArenaLayout, PerRingConfigInitializesRuntimeComponents) {
    ...
    int32_t dep_caps[PTO2_MAX_RING_DEPTH] = {4, 8, 16, 32, 4, 8, 16, 32, 4, 8, 16, 32};
    const uint64_t sm_size = PTO2SharedMemoryHandle::calculate_size_per_ring(ws);
...
TEST(RuntimeArenaLayout, RejectsOverflowingPerRingHeapSum) {
    uint64_t ws[PTO2_MAX_RING_DEPTH] = {16, 32, 64, 128, 16, 32, 64, 128, 16, 32, 64, 128};
    uint64_t heaps[PTO2_MAX_RING_DEPTH] = {std::numeric_limits<uint64_t>::max(), 1};
    int32_t dep_caps[PTO2_MAX_RING_DEPTH] = {4, 8, 16, 32, 4, 8, 16, 32, 4, 8, 16, 32};
