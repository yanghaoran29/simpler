...
protected:
    ...
    std::atomic<uint32_t> &advance_mask(int32_t ring_id) {
        return sched.advance_pending_masks[PTO2SchedulerState::request_mask_index_for_ring(ring_id)].bits;
    }

    std::atomic<uint32_t> &request_mask(int32_t ring_id) {
        return *sched.publication_request_mask_for_ring(ring_id);
    }

    std::atomic<uint32_t> &ack_mask(int32_t ring_id) { return *sched.publication_ack_mask_for_ring(ring_id); }

    bool drain_pending(int32_t ring_id) {
        return sched.drain_pending_ring_advances(task_ring_domain(ring_id), false);
    }
...
TEST_F(SchedulerStateTest, PendingRingAdvanceDrainIsIsolatedByDomain) {
    ...
    uint32_t die0_bit = PTO2SchedulerState::ring_advance_pending_bit(die0_ring);
    uint32_t die1_bit = PTO2SchedulerState::ring_advance_pending_bit(die1_ring);
