...
TEST_F(OrchestratorFaninTest, ExplicitTaskDomainPinsRootAndLiveFaninTasks) {
    ...
    auto submit_scope_chain = [&](TaskReadyDomain domain) {
        ...
        EXPECT_EQ(producer.task_id().ring(), task_ring_id(domain, 0));
        EXPECT_EQ(consumer.task_id().ring(), task_ring_id(domain, 0));
...
TEST_F(OrchestratorFaninTest, NestedScopeUsesPerTaskDomainAndPreservesTensorMapDependencies) {
    ...
    TaskOutputTensors consumer = orch.submit_task(consumer_kernel, consumer_args);
    ASSERT_TRUE(consumer.task_id().is_valid());
    EXPECT_EQ(producer.task_id().ring(), task_ring_id(TaskReadyDomain::DIE0, 1));
    EXPECT_EQ(consumer.task_id().ring(), task_ring_id(TaskReadyDomain::DIE0, 1));
...
TEST_F(OrchestratorFaninTest, UnspecifiedManualScopeUsesGlobalDomain) {
    ...
    EXPECT_EQ(first.task_id().ring(), task_ring_id(TaskReadyDomain::GLOBAL, 0));
    EXPECT_EQ(second.task_id().ring(), task_ring_id(TaskReadyDomain::GLOBAL, 0));
...
TEST_F(OrchestratorFaninTest, OppositeExplicitDieDependencyIsNotCheckedOnSubmitHotPath) {
    ...
    EXPECT_EQ(producer.task_id().ring(), task_ring_id(TaskReadyDomain::DIE0, 0));
    EXPECT_EQ(consumer.task_id().ring(), task_ring_id(TaskReadyDomain::DIE1, 1));
