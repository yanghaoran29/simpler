...
__attribute__((visibility("default"))) void aicpu_orchestration_entry(const L2TaskArgs &orch_args) {
    ...
    // Two-pass emission (PR906 F9 / tip fcc4e793): pre-emit ALL QKs in an
    // outer scope so AICore can start its first wave immediately, then run
    // per-(b,q) SF/PV/UP referencing already-emitted QK outputs.
    ...
    PTO2_SCOPE() {  // outer scope: pre-emit QKs
        ...
        // === Pass 2: per (b,q) SF/PV/UP referencing outer-scope QK outputs ===
        size_t bq_idx = 0;
        L0TaskArgs params_sf, params_pv, params_up;
        for (uint64_t b_idx = 0; b_idx < batch; b_idx++) {
            uint32_t cl_idx[1] = {static_cast<uint32_t>(b_idx)};
            uint64_t cur_seq = static_cast<uint64_t>(get_tensor_data<int32_t>(context_lens, 1, cl_idx));
            uint64_t bn_this_batch = (cur_seq + block_size - 1) / block_size;
