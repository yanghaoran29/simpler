...
/**
 * SPMD MIX slow kernel (AIC): writes float(block_idx) at cache line
 * (base_cl + block_idx * 3 + 0), then optionally spins so the task holds its
 * cluster long enough for a dependent sync_start cohort to pre-stage while it runs.
 *
